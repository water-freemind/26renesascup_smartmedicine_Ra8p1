...
/* CAN 发送诊断（J-Link 直读）：write 非 SUCCESS 次数与最后错误码 */
volatile uint32_t s_dbg_zdt_send_err_count = 0U;
volatile uint32_t s_dbg_zdt_send_err_code = 0U;
...
static fsp_err_t ZDT_Send_Raw(uint32_t id, uint8_t *data, uint8_t total_len)
{
    ...
    while (processed < payload_len || total_len == 1)
    {
        ...
        do 
        {
            err = g_canfd0.p_api->write(g_canfd0.p_ctrl, mb_idx, &frame);
        ...
        if (err == FSP_SUCCESS) {
            ...
            s_dbg_zdt_send_err_count++;
            s_dbg_zdt_send_err_code = (uint32_t) err; /* 发送失败：记录错误码 */
...
{
    return g_canfd0.p_api->open(g_canfd0.p_ctrl, g_canfd0.p_cfg);
}
...
void ZDT_Unprotect(uint32_t id)
{
    /* 解除堵转保护：{0x0E, 0x52, 0x6B}（撞限位/堵转后必须先解除才能再动） */
    uint8_t data[3] = {0x0EU, 0x52U, 0x6BU};
    ZDT_Send_Raw(id, data, 3);
}