...
/* Legacy ZDT application position state.  Keep the tested driver protocol
 * untouched and provide the single storage definition for its public state. */
volatile uint16_t x_position     = 0U;
volatile uint16_t y_position     = 0U;
volatile uint16_t z_position     = 0U;
volatile uint16_t catch_position = 0U;
...
void getMedicine(float x,float y,uint16_t width,uint8_t catch_strength){
    (void)catch_strength;  /* 保留参数签名，当前未使用 */
