...
/* Vendor ID */
#define USB_VENDORID                    (0x045BU)   /* Renesas 官方 VID：VID=0 时 Windows 不安装串口驱动，无法分配 COM 口 */
/* Product ID */
...
/* iSerialNumber
 * 注意：序列号改变会使 Windows 生成全新设备实例(USB\VID_045B&PID_0002\<SN>)，usbser
 * 以全新状态加载，等效于物理拔插——用于清除 usbser 对旧实例缓存的失败打开状态。 */
